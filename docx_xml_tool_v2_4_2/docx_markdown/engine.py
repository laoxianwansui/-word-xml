...
class DocxCore:
    ...
    def apply_block_replacements(self, original_docx: Path, content_map: Path, replacements: Dict[str, str], output_docx: Path) -> ApplyResult:
        original_docx = Path(original_docx)
        content_map = Path(content_map)
        output_docx = Path(output_docx)
        self._validate_docx(original_docx)
        ...
        if not isinstance(blocks, list):
            raise DocxCoreError("content_map.json缺少有效的blocks列表，无法安全回写。")
        block_defs = {item["block_id"]: item for item in blocks}
        unknown = sorted(set(replacements) - set(block_defs))
        if unknown:
            raise DocxCoreError(f"字段回写包含content_map.json无法识别的block：{', '.join(unknown)}")

        full_replacements = {str(block.get("block_id", "")): str(block.get("text", "")) for block in blocks}
        full_replacements.update(replacements)
        touched_parts = {block_defs[block_id]["part"] for block_id in replacements}
        result = self._build_apply_result(blocks, full_replacements)
        output_docx.parent.mkdir(parents=True, exist_ok=True)
        try:
            replacements_by_part: Dict[str, bytes] = {}
            with zipfile.ZipFile(original_docx, mode="r") as archive:
                for part in sorted(touched_parts):
                    root = self._read_required_xml(archive, part)
                    self._apply_part(root, part, block_defs, full_replacements)
                    replacements_by_part[part] = ET.tostring(root, encoding="utf-8", xml_declaration=True)
                self._rewrite_zip(archive, output_docx, replacements_by_part)
        except zipfile.BadZipFile as exc:
            raise DocxCoreError("输出DOCX写入失败，原文件可能已损坏。") from exc
            ...
            if child.tag == w_tag("sdt"):
                ...
                if text.strip():
                    ...
                    blocks.append(Block(
                        ...
                        part_name,
                        text,
                        path=f"sdt[{content_control_index}]",
                        metadata={"content_control_index": content_control_index, "risk": "high"},
                    ))
                    block_counter += 1
                content_control_index += 1
            elif child.tag == w_tag("p"):
                text = self._element_text(child)
                if text.strip():
                    block_id = f"b{block_counter:05d}"
    ...
    def _apply_part(self, root: ET.Element, part: str, block_defs: Dict[str, Dict[str, object]], replacements: Dict[str, str]) -> None:
        ...
        content_controls = [child for child in list(body_like) if child.tag == w_tag("sdt")]
        ...
        for block_id, text in replacements.items():
            ...
            if kind == "paragraph":
                ...
                index = block.get("metadata", {}).get("content_control_index")
                if isinstance(index, int) and index < len(content_controls):
                    self._replace_element_text(content_controls[index], text)
            elif kind == "footnote":
