...
`03_fill_fields_to_docx.bat` is the second-step field-fill launcher. It expects an exported workspace folder, not a raw `.docx` file. The workspace must contain `original.docx`, `content_map.json`, `fields.yaml`, and `field_values.yaml`. First run `01_export_docx_workspace.bat`, then edit `field_values.yaml`, then drag the whole workspace folder onto `03_fill_fields_to_docx.bat`. The generated DOCX is written under the workspace `edits/` folder.
...
v2.4.2 adds a field-oriented editing path for template-style Word documents.

Important: `03_fill_fields_to_docx.bat` does not accept a raw `.docx` file. It accepts the workspace folder created by `01_export_docx_workspace.bat`.

Complete workflow:

1. Drag the raw `.docx` onto:

   ```text
   launchers/01_export_docx_workspace.bat
   ```

2. Open the generated workspace and edit:
...
3. Drag the whole workspace folder onto:

   ```text
   launchers/03_fill_fields_to_docx.bat
   ```

You can also run step 3 from the command line:
...
```text
workspace/edits/field_fill_output_001.docx
workspace/edits/field_fill_audit_001.md
```

Field fill is conservative. Ambiguous duplicate fields are skipped and reported in the audit instead of being silently edited.
...
| `fields.yaml` | v2.4.2 field-fill candidates generated from mapped blocks. |
| `field_values.yaml` | User-editable values for `--fill-fields`. |
| `document.md` | Readable Markdown editing surface. |
...
- Keep the distributable tool folder named `docx_xml_tool` or `docx_xml_tool_v2_4_2`.
...
- Field fill outputs use `edits/field_fill_output_001.docx`, `edits/field_fill_output_002.docx`, and so on.
...
## v2.4.2 stability scope

v2.4.2 is a stabilization release before the GUI work. It improves the safety boundary for complex Word files without claiming full Word layout understanding.
...
Complex Word files can contain drawing layers, text boxes, repeated visible text, hidden XML order, merged tables, headers, footers, notes, images, comments, and revision data. v2.4.2 handles this by being conservative:
...
Keep `content_map.json` if you may ever write changes back to DOCX. Full multi-document RAG, vector indexes, and database-backed search are planned for a later version, not v2.4.2.

## Not implemented in v2.4.2
