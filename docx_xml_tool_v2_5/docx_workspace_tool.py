...
from docx_markdown.fields import detect_fields_from_blocks, parse_field_values, plan_field_replacements, write_field_files
...
def write_v24_markdown_outputs(input_docx, output_path):
    core = DocxCore()
    result = core.export(Path(input_docx), Path(output_path))
    write_field_files(Path(output_path), result.content_map)
...
def next_field_fill_paths(workspace_dir, output_docx=None):
    edits_dir = workspace_dir / "edits"
    edits_dir.mkdir(parents=True, exist_ok=True)
    existing = sorted(edits_dir.glob("field_fill_output_*.docx"))
    next_number = len(existing) + 1
    audit_path = edits_dir / f"field_fill_audit_{next_number:03d}.md"
    if output_docx:
        output_path = Path(output_docx).resolve()
    else:
        output_path = edits_dir / f"field_fill_output_{next_number:03d}.docx"
    return edits_dir, output_path, audit_path


def fill_fields_workspace(workspace_dir, field_values_path=None, output_docx=None):
    workspace_dir = Path(workspace_dir).resolve()
    original_docx = workspace_dir / "original.docx"
    content_map = workspace_dir / "content_map.json"
    fields_path = workspace_dir / "fields.yaml"
    values_path = Path(field_values_path).resolve() if field_values_path else workspace_dir / "field_values.yaml"
    if not original_docx.exists():
        raise FileNotFoundError(f"找不到 original.docx：{original_docx}")
    if not content_map.exists():
        raise FileNotFoundError(f"找不到 content_map.json：{content_map}")
    if not fields_path.exists():
        raise FileNotFoundError(f"找不到 fields.yaml：{fields_path}")
    if not values_path.exists():
        raise FileNotFoundError(f"找不到 field_values.yaml：{values_path}")

    mapping = json.loads(content_map.read_text(encoding="utf-8"))
    fields = detect_fields_from_blocks(mapping.get("blocks", []))
    values = parse_field_values(values_path.read_text(encoding="utf-8"))
    plan = plan_field_replacements(fields, values, mapping.get("blocks", []))
    edits_dir, output_path, audit_path = next_field_fill_paths(workspace_dir, output_docx)
    core = DocxCore()
    core.apply_block_replacements(original_docx, content_map, plan["replacements"], output_path)
    zip_test = "ok"
    with zipfile.ZipFile(output_path) as z:
        bad = z.testzip()
        if bad:
            zip_test = str(bad)
    write_field_fill_audit(audit_path, workspace_dir, values_path, output_path, plan, zip_test)
    return output_path


def write_field_fill_audit(audit_path, workspace_dir, values_path, output_path, plan, zip_test):
    audit_lines = [
        "# Field Fill Audit",
        "",
        f"Workspace: {workspace_dir}",
        f"Field values: {values_path}",
        f"Output DOCX: {output_path}",
        "",
        "## Summary",
        "",
        f"changed_fields: {len(plan['changed_fields'])}",
        f"skipped_fields: {len(plan['skipped_fields'])}",
        f"warnings: {len(plan['warnings'])}",
        "",
    ]
    if plan["changed_fields"]:
        audit_lines.extend(["## Changed fields", ""])
        for field in plan["changed_fields"]:
            audit_lines.extend([
                f"### {field['key']}",
                "",
                f"block_id: {field['block_id']}",
                "before:",
                "```text",
                str(field["old_text"]),
                "```",
    ...
    if plan["skipped_fields"]:
        audit_lines.extend(["## Skipped fields", ""])
        for field in plan["skipped_fields"]:
            audit_lines.append(f"- {field['key']}: {field['reason']}")
        audit_lines.append("")
    if plan["warnings"]:
        audit_lines.extend(["## Warnings", ""])
        audit_lines.extend(f"- {warning}" for warning in plan["warnings"])
        audit_lines.append("")
    audit_lines.extend(["## Verification", "", f"zip_test: {zip_test}", ""])
    audit_path.write_text("\n".join(audit_lines), encoding="utf-8")
    return output_path
...
def main(argv=None):
    ...
    parser.add_argument("--fill-fields", action="store_true", help="把工作区 field_values.yaml 填入新的 DOCX")
    parser.add_argument("--field-values", help="--fill-fields 时指定字段值 YAML 路径")
    parser.add_argument("--output", help="--apply-md 或 --fill-fields 时指定输出 DOCX 路径")
    parser.add_argument("--web-gui", action="store_true", help="启动本地 Web GUI")
    parser.add_argument("--host", default="127.0.0.1", help="Web GUI 监听地址，默认只监听本机")
    parser.add_argument("--port", type=int, default=8765, help="Web GUI 监听端口")
    parser.add_argument("--no-open", action="store_true", help="启动 Web GUI 时不自动打开浏览器")
    ...
    if args.test:
        ...
        return
    if args.web_gui:
        from docx_web_gui import run_server
        run_server(host=args.host, port=args.port, open_browser=not args.no_open, base_dir=Path(__file__).resolve().parent)
        return
    if not args.input:
        parser.print_help()
        return
    if args.apply_md and args.fill_fields:
        print("错误：--apply-md 和 --fill-fields 不能同时使用", file=sys.stderr)
        raise SystemExit(2)
    if args.fill_fields:
        try:
            output_path = fill_fields_workspace(args.input, args.field_values, args.output)
            print(f"字段填充完成：{output_path}")
