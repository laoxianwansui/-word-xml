...
| v2.4.1 | previous release | superseded | 更严格的 Markdown 标记校验、更详细审计、尾注支持、复杂 Word 回归测试、发布打包脚本。 |
| v2.4.2 | `docx_xml_tool_v2_4_2/` | current stable core | v2.4.1 plus Smart Field Fill：`fields.yaml` / `field_values.yaml`、字段填充输出、字段审计、重复风险提示、内容控件高风险识别。 |
...
v2.4.2 相比 v2.4.1 增强了模板填写能力，同时保留原有 Markdown 安全回写边界：
...
- 导出 `fields.yaml` 和 `field_values.yaml`，支持模板式字段填充。
- `--fill-fields` 生成 `field_fill_output_###.docx` 和字段审计报告。
- 对重复/不明确字段保守跳过，并在审计中提示。
...
```bash
python docx_xml_tool_v2_4_2/test_docx_workspace_tool.py
python docx_xml_tool_v2_4_2/docx_workspace_tool.py --test
```
