# DOCX XML Tool v2.4.2（DOCX XML 导出与 Markdown 回写工具）
...
`03_fill_fields_to_docx.bat` 是字段填充的第二步脚本，需要拖入一个已经导出的工作区文件夹，不能直接拖原始 .docx。这个工作区文件夹里必须包含 `original.docx`、`content_map.json`、`fields.yaml` 和 `field_values.yaml`。正确流程是：先用 `01_export_docx_workspace.bat` 导出工作区，再编辑 `field_values.yaml`，最后把整个工作区文件夹拖到 `03_fill_fields_to_docx.bat`。新 Word 会生成在该工作区的 `edits/` 目录中。
...
v2.4.2 新增面向模板类 Word 文档的字段填充流程。

注意：`03_fill_fields_to_docx.bat` 不能直接拖原始 .docx。它接收的是已经由 `01_export_docx_workspace.bat` 导出的工作区文件夹。
...
字段填充采用保守策略。遇到重复字段或不明确位置时，不会静默乱改，而是在审计文件中提示并跳过。
...
| `fields.yaml` | v2.4.2 根据映射 block 生成的字段填充候选。 |
| `field_values.yaml` | 用户编辑后供 `--fill-fields` 使用的字段值文件。 |
...
- 对外发布的工具目录建议命名为 `docx_xml_tool` 或 `docx_xml_tool_v2_4_2`。
...
- 字段填充输出命名为 `edits/field_fill_output_001.docx`、`edits/field_fill_output_002.docx`，依次递增。
...
v2.4.2 是进入 GUI 前的稳定收口版本。它重点增强复杂 Word 文件的安全边界，但不承诺完整理解所有 Word 版式。
...
复杂 Word 可能包含绘图层、文本框、重复可见文本、隐藏 XML 顺序、合并表格、页眉、页脚、脚注、尾注、图片、批注和修订。v2.4.2 的策略是保守处理：
...
如果未来还要回写 DOCX，必须保留 `content_map.json`。多文档 RAG、向量索引、数据库检索属于后续版本，不属于 v2.4.2。
