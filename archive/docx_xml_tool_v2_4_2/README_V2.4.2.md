# DOCX XML Tool v2.4.2 Overview

这是在第一版 `docx_xml_tool` 基础上叠加 Markdown 编辑/回写能力的 v2.4.2 版本。核心原则不变：不重建 Word 文档，不修改原始 DOCX，只基于原 DOCX 包和 XML 映射生成新的 `.docx`。
...
或编辑 field_values.yaml
  ↓
拖工作区到 launchers/03_fill_fields_to_docx.bat 或运行 --fill-fields
  ↓
生成 edits/field_fill_output_001.docx 和字段审计文件
```
...
v2.4.2 是进入 GUI 前的稳定收口版本，重点不是“自动理解所有复杂 Word”，而是更安全地识别风险、拒绝危险回写、保留未编辑包内容，并让审计文件可追踪。
...
- 对外发布目录建议命名为 `docx_xml_tool` 或 `docx_xml_tool_v2_4_2`。
...
- 字段填充输出命名规则：`edits/field_fill_output_001.docx`、`edits/field_fill_output_002.docx`，依次递增。
